...
function run() {
  var ctx = document.getElementById('scene').getContext('2d'),
      spirals = [
        ...
        new Spiral({
          foreground: "#FFD700", // nampiako teo 
          angleOffset: Math.PI,
          factor: factor
        }),
        new Spiral({
  ...
  function renderFrame() {
    ...
    //une étoile
    drawStar(ctx, 260, 50, 5, 50, 15, "#FFD700");
    drawFestiveText(ctx);
  }
...
function drawFestiveText(ctx) {
  ctx.font = "bold 50px 'Cinzel', sans-serif";

  //  un dégradé 
  let gradient = ctx.createLinearGradient(150, 400, 350, 400);
  gradient.addColorStop(0, "#00ffcc"); 
  gradient.addColorStop(1, "red"); 

  ctx.fillStyle = gradient; 
  ctx.textAlign = "center"; // Centrer le texte
  ctx.fillText("Joyeux Noel ", 250, 500); // Dessiner le texte

  //  un contour doré pour l'effet festif
  ctx.strokeStyle = "#FFD700"; 
  ctx.lineWidth = 2; 
  ctx.strokeText("Joyeux Noel ", 250, 500); 
}
